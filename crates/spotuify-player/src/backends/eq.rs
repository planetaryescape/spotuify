...
struct EqShared {
    ...
    /// Gain reduction the limiter published for the last packet, in tenths
    /// of a dB. Written by the audio thread, read by anyone.
    limiting_tenths: AtomicU16,
}
...
impl SharedEq {
    pub fn new() -> Self {
        Self(Arc::new(EqShared {
            ...
            limiting_tenths: AtomicU16::new(0),
        }))
    }
    ...
    fn publish_limiting(&self, limiting: EqLimiting) {
        self.0
            .limiting_tenths
            .store(limiting.into_tenths(), Ordering::Relaxed);
    }
}
...
impl EqLimiterMeter {
    ...
    pub fn limiting(&self) -> EqLimiting {
        EqLimiting::from_tenths(self.0.limiting_tenths.load(Ordering::Relaxed))
    }
}
...
mod tests {
    ...
    fn settled(stage: &mut EqStage, eq: &SharedEq, mut signal: Vec<f64>) -> Vec<f64> {
    ...
    fn settled_sine(stage: &mut EqStage, eq: &SharedEq, hz: f64, amplitude: f64) -> Vec<f64> {
        settled(stage, eq, sine(hz, SAMPLE_RATE as usize / 2, amplitude))
    }
    ...
    fn steady_state_gain(stage: &mut EqStage, eq: &SharedEq, hz: f64) -> f64 {
        ...
        rms(&settled_sine(stage, eq, hz, 0.5)) / rms(&reference)
    }
    ...
    fn a_boosted_band_lifts_its_own_frequency_and_leaves_the_others() {
        ...
        let at_1k = rms(&settled_sine(&mut stage, &eq, 1_000.0, 0.2)) / rms(&reference);
        assert!(
            (at_1k - 4.0).abs() < 0.4,
        ...
        let at_100 = rms(&settled_sine(&mut stage, &eq, 100.0, 0.2)) / rms(&reference);
        assert!(
            (at_100 - 1.0).abs() < 0.1,
    ...
    fn a_boost_preset_no_longer_quietens_the_bands_it_does_not_touch() {
        ...
        let (mut flat_stage, flat_eq) = stage_and_eq(&EqSettings::flat());
        let flat = steady_state_gain(&mut flat_stage, &flat_eq, UNTOUCHED_HZ);

        let bass = EqSettings::from_preset("Bass Boost").unwrap();
        let (mut stage, eq) = stage_and_eq(&bass);
        let boosted = steady_state_gain(&mut stage, &eq, UNTOUCHED_HZ);
    ...
    fn a_boost_preset_keeps_a_quiet_broadband_signal_at_its_own_level() {
        ...
        let (mut flat_stage, flat_eq) = stage_and_eq(&EqSettings::flat());
        let flat = rms(&settled(&mut flat_stage, &flat_eq, signal.clone()));

        for preset in ["Bass Boost", "Rock", "Loudness", "Electronic"] {
            let settings = EqSettings::from_preset(preset).unwrap();
            let (mut stage, eq) = stage_and_eq(&settings);
            let out = rms(&settled(&mut stage, &eq, signal.clone()));
    ...
    fn a_full_scale_sine_through_a_boost_preset_stays_inside_full_scale() {
        ...
        for (name, _) in spotuify_core::EQ_PRESETS {
            ...
            let (mut stage, eq) = stage_and_eq(&settings);
            let out = settled_sine(&mut stage, &eq, 76.0, 1.0);
            assert!(
    ...
    fn the_limiter_engages_on_a_loud_boost_and_stays_idle_on_a_quiet_one() {
        ...
        let (mut stage, eq) = stage_and_eq(&bass);
        let out = settled_sine(&mut stage, &eq, 70.0, 0.3);
    ...
    fn the_limiter_releases_to_unity_once_the_transient_passes() {
        ...
        stage.process(&eq, &mut transient);
        assert!(
