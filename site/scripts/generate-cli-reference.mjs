...
const EXPECTED_PAGES = [
  ...
  'analytics-export',
  'analytics-import',
  'analytics-import-lastfm',
  'analytics-import-status',
  'analytics-import-unresolved',
  'analytics-import-undo',
  'ops',
  'ops-log',
  'ops-show',
  'ops-undo',
...
const COMMAND_EXAMPLES = {
  ...
  'playlist-remove': ['spotuify playlist remove "Quiet Storm" spotify:track:... --dry-run'],
  'playlist-remove-at': ['spotuify playlist remove-at "Quiet Storm" 2 5 --dry-run', 'spotuify playlist remove-at "Quiet Storm" 2 5 --yes'],
  ...
  speed: ['spotuify speed', 'spotuify speed 1.5x', 'spotuify speed +', 'spotuify speed --format json'],
  eq: ['spotuify eq', 'spotuify eq presets', "spotuify eq 'bass boost'", 'spotuify eq --band 4 -3', 'spotuify eq --reset', 'spotuify eq --format json'],
  bookmark: ['spotuify bookmark add', 'spotuify bookmark list --current', 'spotuify bookmark play <bookmark-id>'],
  'bookmark-add': ['spotuify bookmark add', 'spotuify bookmark add --note "great bit"', 'spotuify bookmark add --uri spotify:episode:... --at 1:02:03'],
  'bookmark-list': ['spotuify bookmark list', 'spotuify bookmark list --current', 'spotuify bookmark list --format json'],
  'bookmark-note': ['spotuify bookmark note <bookmark-id> "re-listen with notes"', 'spotuify bookmark note <bookmark-id>'],
  'bookmark-delete': ['spotuify bookmark delete <bookmark-id>'],
  'bookmark-play': ['spotuify bookmark play <bookmark-id>'],
  ...
  'viz-style': ['spotuify viz style', 'spotuify viz style classic-peak', 'spotuify viz style next'],
  'viz-styles': ['spotuify viz styles', 'spotuify viz styles --format ids'],
  ...
  'analytics-import-status': ['spotuify analytics import status 018f... --format json'],
  'analytics-import-unresolved': ['spotuify analytics import unresolved 018f... --format json'],
  'analytics-import-undo': [
...
const EXTRA_HELP = {
  ...
  'analytics-import': manualHelp({
    about: 'Import historical scrobbles',
    ...
    commands: [
      ...
      ['undo', 'Undo promoted analytics facts for a run'],
      ['help', 'Print this message or the help of the given subcommand(s)'],
    ],
    options: [
      '      --target <TARGET>  Compatibility import target; only lastfm is implemented [possible values: listenbrainz, lastfm]',
      '      --format <FORMAT>  Output format [default: table] [possible values: table, json, jsonl, csv, ids]',
    ],
  }),
  'analytics-import-lastfm': manualHelp({
    about: 'Preview/apply Last.fm historical scrobble import',
    usage: 'spotuify analytics import lastfm [OPTIONS]',
    options: [
      '      --user <USER>        Last.fm username; overrides config/env',
      '      --api-key <API_KEY>  Last.fm API key; overrides config/env',
      '      --from <FROM>        Start date or timestamp',
      '      --to <TO>            End date or timestamp',
      '      --apply              Write raw scrobbles and promoted listen facts; default is dry-run',
      '      --format <FORMAT>    Output format [default: table] [possible values: table, json, jsonl, csv, ids]',
    ],
  }),
  'analytics-import-status': manualHelp({
    about: 'Show import run status',
    usage: 'spotuify analytics import status [OPTIONS] <RUN_ID>',
    args: ['  <RUN_ID>  Import run id'],
    options: [
      '      --format <FORMAT>  Output format [default: table] [possible values: table, json, jsonl, csv, ids]',
    ],
  }),
  'analytics-import-unresolved': manualHelp({
    about: 'List unresolved scrobbles for a run',
  ...
  'analytics-import-undo': manualHelp({
    about: 'Undo promoted analytics facts for a run',
    usage: 'spotuify analytics import undo [OPTIONS] <RUN_ID>',
    args: ['  <RUN_ID>  Import run id'],
    options: [
      '      --dry-run          Preview removed promoted listen facts',
      '      --yes              Apply undo',
      '      --format <FORMAT>  Output format [default: table] [possible values: table, json, jsonl, csv, ids]',
    ],
  }),
