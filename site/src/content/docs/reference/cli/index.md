...
Commands:
  ...
  providers       List configured provider adapters and their capabilities
  search          Search a provider catalog or the local cache
  ...
  queue           Print the current playback queue
  playlists       List the current user's playlists
  history         Listening history grouped into sessions. Use --flat for a chronological track list
  play            Search a provider and play the first matching result. Recognized resource references skip search and play directly
  play-uri        Play a provider resource directly
  resolve         Resolve a raw target (URI, share link, or bare ID) to its canonical provider reference. Prints the resolved target, nothing when the input is not a recognized reference, or an error when the provider rejects it
  ...
  bookmark        Save and revisit positions inside podcasts and tracks
  refresh-media   Refresh current track cover art and lyrics
  theme           Show, list, or switch the terminal colour theme
  ...
  sync            Refresh the local cache from a provider
