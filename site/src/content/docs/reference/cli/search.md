...
Options:
      ...
      --source <SOURCE>
          Where to search. `spotify` (default) queries the Web API for catalog discovery. `local` queries only the local Tantivy index (offline / library lookup). `hybrid` returns local cached hits immediately and refreshes Spotify in the background

          [default: spotify]
          [possible values: local, spotify, hybrid]
