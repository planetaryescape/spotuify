...
| `ThemesList` / `SetTheme` | `spotuify theme` / `theme list` / `theme <name>` / `theme path`; TUI `t` picker; MCP `themes_list` / `theme_set`. Merges built-ins with `<config_dir>/themes/*.toml`, writes `tui.theme`, then broadcasts `ClientPreferencesChanged` carrying the RESOLVED colours so no client reads a theme file. The `themes` response carries `active` as a whole spec, not a name: the daemon keeps painting a theme whose file was deleted mid-session, so the active theme is not always in the list. `SetTheme` shares a write lane with `SetVizStyle` so two preference writes cannot leave the config file and the broadcast disagreeing |
